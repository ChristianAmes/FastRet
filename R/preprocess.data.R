...
preprocess.data <- function(data, preprocessed = F, include_HMDB =F,
                            clean_data = T, include_polynomial = F, 
                            degree_polynomial = 2, interaction_terms = F) {
  ...
  if (preprocessed) {
    ...
    if (clean_data) {
      ...
      db_rt[which(is.na(db_rt[, "Kier3"])), "Kier3"] <- mean(db_rt[, "Kier3"], na.rm = T)
      db_rt <- db_rt[, !apply(db_rt, 2, function(x) any(is.na(x)))]
      db_rt <- db_rt[, -which(colnames(db_rt) %in%
                                c("NAME", "SMILES", "InChIKey","INCHKEY"))]
